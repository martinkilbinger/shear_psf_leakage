...
class CovTauTh:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.rho_3_p_itp = (
            self.build_interpolator(self.size_error, self.size_error, type="plus")
            if self.use_eta
            else None
        )
        self.rho_3_m_itp = (
            self.build_interpolator(self.size_error, self.size_error, type="minus")
            if self.use_eta
            else None
        )
        self.rho_4_p_itp = (
            self.build_interpolator(self.psf_error, self.size_error, type="plus")
            if self.use_eta
            else None
        )
        self.rho_4_m_itp = (
            self.build_interpolator(self.psf_error, self.size_error, type="minus")
            if self.use_eta
            else None
        )
        self.rho_5_p_itp = (
        ...
        dummy_cat = treecorr.Catalog(
            ra=[0], dec=[0], g1=[0], g2=[0], w=[1], ra_units="deg", dec_units="deg"
        )
    ...
    def create_treecorr_catalog(self, cat_gal, cat_psf):
        ...
        if isinstance(self._params["R11"], str):
            ...
            e1_gal = cat_gal[self._params["e1_col"]] - np.average(
                cat_gal[self._params["e1_col"]], weights=cat_gal[self._params["w_col"]]
            )
            e2_gal = cat_gal[self._params["e2_col"]] - np.average(
                cat_gal[self._params["e2_col"]], weights=cat_gal[self._params["w_col"]]
            )
    ...
    def compute_cv_rho_plus(self, component, **kwargs):
        ...
        for i in range(len(self.bins)):
            for j in range(len(self.bins)):
                ...
                cv[i, j] = (
                    integrate.simpson(radius_val, phi_radius) * 1 / (np.pi * self.A)
                )
        return cv
        ...
        for i in range(len(self.bins)):
            for j in range(len(self.bins)):
                ...
                int_tau_b_cos = integrate.simpson(
                    interpolator_tau_b(norm_a) * np.cos(4 * polar_a), phi_angle, axis=2
                )
                int_tau_c_cos = integrate.simpson(
                    interpolator_tau_c(norm_b) * np.cos(4 * polar_b), phi_angle, axis=2
                )
                int_tau_b_sin = integrate.simpson(
                    interpolator_tau_b(norm_a) * np.sin(4 * polar_a), phi_angle, axis=2
                )
                int_tau_c_sin = integrate.simpson(
                    interpolator_tau_c(norm_b) * np.sin(4 * polar_b), phi_angle, axis=2
                )
                int_xi_cos = integrate.simpson(
                    interpolator_xi(norm_a) * np.cos(4 * polar_a), phi_angle, axis=2
                )
                int_rho_cos = integrate.simpson(
                    interpolator_rho(norm_b) * np.cos(4 * polar_b), phi_angle, axis=2
                )
                int_xi_sin = integrate.simpson(
                    interpolator_xi(norm_a) * np.sin(4 * polar_a), phi_angle, axis=2
                )
                int_rho_sin = integrate.simpson(
                    interpolator_rho(norm_b) * np.sin(4 * polar_b), phi_angle, axis=2
                )
