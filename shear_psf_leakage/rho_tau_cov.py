...
class CovTauTh:
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Load the catalogs
        cat_gal, cat_psf = (
            fits.getdata(path_gal),
            fits.open(path_psf)[hdu_psf].data,
        )
        ...
        self.gal, self.psf, self.psf_error, self.size_error = (
            self.create_treecorr_catalog(cat_gal, cat_psf)
        )
        ...
        self.rho_0_p_itp = self.build_interpolator(
            self.psf, self.psf, type="plus"
        )
        self.rho_0_m_itp = self.build_interpolator(
            self.psf, self.psf, type="minus"
        )
        self.rho_1_p_itp = self.build_interpolator(
            self.psf_error, self.psf_error, type="plus"
        )
        self.rho_1_m_itp = self.build_interpolator(
            self.psf_error, self.psf_error, type="minus"
        )
        self.rho_2_p_itp = self.build_interpolator(
            self.psf, self.psf_error, type="plus"
        )
        self.rho_2_m_itp = self.build_interpolator(
            self.psf, self.psf_error, type="minus"
        )
        self.rho_3_p_itp = (
            self.build_interpolator(
                self.size_error, self.size_error, type="plus"
            )
            if self.use_eta
            else None
        )
        self.rho_3_m_itp = (
            self.build_interpolator(
                self.size_error, self.size_error, type="minus"
            )
            if self.use_eta
            else None
        )
        self.rho_4_p_itp = (
            self.build_interpolator(
                self.psf_error, self.size_error, type="plus"
            )
            if self.use_eta
            else None
        )
        self.rho_4_m_itp = (
            self.build_interpolator(
                self.psf_error, self.size_error, type="minus"
            )
            if self.use_eta
            else None
        )
        self.rho_5_p_itp = (
            self.build_interpolator(self.psf, self.size_error, type="plus")
            if self.use_eta
            else None
        )
        self.rho_5_m_itp = (
            self.build_interpolator(self.psf, self.size_error, type="minus")
            if self.use_eta
            else None
        )

        # interpolate tau stats
        self.tau_0_p_itp = self.build_interpolator(
            self.gal, self.psf, type="plus"
        )
        self.tau_0_m_itp = self.build_interpolator(
            self.gal, self.psf, type="minus"
        )
        self.tau_2_p_itp = self.build_interpolator(
            self.gal, self.psf_error, type="plus"
        )
        self.tau_2_m_itp = self.build_interpolator(
            self.gal, self.psf_error, type="minus"
        )
        self.tau_5_p_itp = (
            self.build_interpolator(self.gal, self.size_error, type="plus")
            if self.use_eta
            else None
        )
        self.tau_5_m_itp = (
            self.build_interpolator(self.gal, self.size_error, type="minus")
            if self.use_eta
            else None
        )

        # interpolate xi plus and minus
        self.xi_plus_itp = self.build_interpolator(
            self.gal, self.gal, type="plus"
        )
        self.xi_minus_itp = self.build_interpolator(
            self.gal, self.gal, type="minus"
        )
    ...
    def get_area(self, cat_gal, nside):
        ...
        unique_pix, idx, idx_pix = np.unique(
            pix, return_index=True, return_inverse=True
        )

        n_map[unique_pix] += np.bincount(
            idx_pix, weights=cat_gal[self._params["w_col"]]
        )
    ...
    def get_effective_number_density(self, cat_gal):
        ...
        return (
            1
            / (self.A)
            * (np.sum(cat_gal[self._params["w_col"]])) ** 2
            / np.sum(cat_gal[self._params["w_col"]] ** 2)
        )
    ...
    def create_treecorr_catalog(self, cat_gal, cat_psf):
        ...
        if isinstance(self._params["R11"], str):
            e1_gal = (
                cat_gal[self._params["e1_col"]]
                - np.average(
                    cat_gal[self._params["e1_col"]],
                    weights=cat_gal[self._params["w_col"]],
                )
            ) / np.average(cat_gal[self._params["R11"]])
            e2_gal = (
                cat_gal[self._params["e2_col"]]
                - np.average(
                    cat_gal[self._params["e2_col"]],
                    weights=cat_gal[self._params["w_col"]],
                )
            ) / np.average(cat_gal[self._params["R22"]])
        else:
            e1_gal = cat_gal[self._params["e1_col"]] - np.average(
                cat_gal[self._params["e1_col"]],
                weights=cat_gal[self._params["w_col"]],
            )
            e2_gal = cat_gal[self._params["e2_col"]] - np.average(
                cat_gal[self._params["e2_col"]],
                weights=cat_gal[self._params["w_col"]],
            )
        ...
        psf = treecorr.Catalog(
            ra=cat_psf[self._params["ra_PSF_col"]],
            dec=cat_psf[self._params["dec_PSF_col"]],
            g1=cat_psf[self._params["e1_PSF_col"]],
            g2=cat_psf[self._params["e2_PSF_col"]],
            ra_units=ra_units,
            dec_units=dec_units,
        )
        ...
        psf_error = treecorr.Catalog(
            ra=cat_psf[self._params["ra_PSF_col"]],
            dec=cat_psf[self._params["dec_PSF_col"]],
            g1=cat_psf[self._params["e1_star_col"]]
            - cat_psf[self._params["e1_PSF_col"]],
            g2=cat_psf[self._params["e2_star_col"]]
            - cat_psf[self._params["e2_PSF_col"]],
            ra_units=ra_units,
            dec_units=dec_units,
        )
        ...
        if self.use_eta:
            size_resid = (
                (
                    cat_psf[self._params["star_size"]] ** 2
                    - cat_psf[self._params["PSF_size"]] ** 2
                )
                / cat_psf[self._params["star_size"]] ** 2
                if self._params["square_size"]
                else (
                    cat_psf[self._params["star_size"]]
                    - cat_psf[self._params["PSF_size"]]
                )
                / cat_psf[self._params["star_size"]]
            )
    ...
    def compute_shape_noise(self, cat1, cat2):
        ...
        return 0.5 * np.average(
            (cat1.g1 * cat2.g1) + (cat1.g2 * cat2.g2), weights=cat1.w * cat2.w
        )
    ...
    def build_interpolator(self, cat_1, cat_2, type):
        ...
        assert type in [
            "plus",
            "minus",
        ], "The type must be either 'plus' or 'minus'"

        gg = treecorr.GGCorrelation(
            nbins=30, min_sep=8e-2, max_sep=1e3, sep_units="arcmin"
        )
        ...
        func = lambda x: np.where(
            ...
            np.where(
                ...
                np.zeros_like(
                    x
                ),  # np.where(x < bins[0], np.ones_like(x)*values[0],
                np.zeros_like(x),
            ),
        )
        return func
    ...
    def build_cov(self, **kwargs):
        ...
        if self.use_eta:
            ...
            cov = np.block([[cov_00, cov_02], [cov_02.T, cov_22]])
        return cov
    ...
    def compute_00_comp(self, **kwargs):
        ...
        """
        cov_00 = (
            self.compute_sn("00", **kwargs)
            + self.compute_mt("00", **kwargs)
            + self.compute_cv_rho_plus("00", **kwargs)
    ...
    def compute_02_comp(self, **kwargs):
        ...
        """
        cov_02 = (
            self.compute_sn("02", **kwargs)
            + self.compute_mt("02", **kwargs)
            + self.compute_cv_rho_plus("02", **kwargs)
    ...
    def compute_05_comp(self, **kwargs):
        ...
        """
        cov_05 = (
            self.compute_sn("05", **kwargs)
            + self.compute_mt("05", **kwargs)
            + self.compute_cv_rho_plus("05", **kwargs)
    ...
    def compute_25_comp(self, **kwargs):
        ...
        """
        cov_25 = (
            self.compute_sn("25", **kwargs)
            + self.compute_mt("25", **kwargs)
            + self.compute_cv_rho_plus("25", **kwargs)
    ...
    def compute_22_comp(self, **kwargs):
        ...
        """
        cov_22 = (
            self.compute_sn("22", **kwargs)
            + self.compute_mt("22", **kwargs)
            + self.compute_cv_rho_plus("22", **kwargs)
    ...
    def compute_55_comp(self, **kwargs):
        ...
        """
        cov_55 = (
            self.compute_sn("55", **kwargs)
            + self.compute_mt("55", **kwargs)
            + self.compute_cv_rho_plus("55", **kwargs)
    ...
    def compute_sn(self, component, **kwargs):
        ...
        """
        assert component in self.component_dict.keys(), (
            "The component must be in the following list: %s"
            % self.component_dict.keys()
        )
        ...
        for i in range(len(self.bins)):
            for j in range(len(self.bins)):
                y = np.sqrt(
                    self.bins[i] ** 2
                    + self.bins[j] ** 2
                    - 2 * self.bins[i] * self.bins[j] * np.cos(phi)
                )
                mt[i, j] = (
                    integrate.simpson(interpolator_rho(y), phi)
                    * self.sigma_e
                    / (2 * np.pi * self.A * self.n_e)
                )
                if sigma_bc is not None:
                    mt[i, j] += (
                        integrate.simpson(interpolator_xi_plus(y), phi)
                        * sigma_bc
                        / (2 * np.pi * self.A * self.n_e)
                    )
        return mt
        ...
        for i in range(len(self.bins)):
            for j in range(len(self.bins)):
                ...
                y_m = np.sqrt(
                    phi_radius[:, None] ** 2
                    + self.bins[i] ** 2
                    - 2 * phi_radius[:, None] * self.bins[i] * np.cos(phi_angle)
                )
                y_p = np.sqrt(
                    phi_radius[:, None] ** 2
                    + self.bins[j] ** 2
                    + 2 * phi_radius[:, None] * self.bins[j] * np.cos(phi_angle)
                )
                ...
                radius_val = 0.5 * (
                    int_xi_m * int_rho_p * phi_radius
                    + int_xi_p * int_rho_m * phi_radius
                )
                cv[i, j] = (
                    integrate.simpson(radius_val, phi_radius)
                    * 1
                    / (np.pi * self.A)
                )
        return cv
    ...
    def compute_cv_tau_plus(self, component, **kwargs):
        ...
        for i in range(len(self.bins)):
            for j in range(len(self.bins)):
                ...
                int_tau_b_m = integrate.simpson(
                    interpolator_tau_b(y_m), phi_angle
                )
                int_tau_b_p = integrate.simpson(
                    interpolator_tau_b(y_p), phi_angle
                )
                int_tau_c_m = integrate.simpson(
                    interpolator_tau_c(y_m), phi_angle
                )
                int_tau_c_p = integrate.simpson(
                    interpolator_tau_c(y_p), phi_angle
                )
                radius_val = (
                    0.5
                    * (int_tau_b_m * int_tau_c_p + int_tau_b_p * int_tau_c_m)
                    * phi_radius
                )
                cv[i, j] = (
                    integrate.simpson(radius_val, phi_radius)
    ...
    def compute_cv_minus(self, component, **kwargs):
        ...
        for i in range(len(self.bins)):
            for j in range(len(self.bins)):
                phi_r_mesh, phi_varphi_mesh = np.meshgrid(
                    phi_radius, phi_angle, indexing="ij"
                )

                # Compute psi_a and psi_b in a vectorized manner
                psi_a_x = (phi_r_mesh * np.cos(phi_varphi_mesh))[
                    :, :, None
                ] - self.bins[i] * np.cos(phi_angle)
                psi_a_y = (phi_r_mesh * np.sin(phi_varphi_mesh))[
                    :, :, None
                ] - self.bins[i] * np.sin(phi_angle)
                psi_b_x = (phi_r_mesh * np.cos(phi_varphi_mesh))[
                    :, :, None
                ] + self.bins[j] * np.cos(phi_angle)
                psi_b_y = (phi_r_mesh * np.sin(phi_varphi_mesh))[
                    :, :, None
                ...
                int_tau_b_cos = integrate.simpson(
                    interpolator_tau_b(norm_a) * np.cos(4 * polar_a),
                    phi_angle,
                    axis=2,
                )
                int_tau_c_cos = integrate.simpson(
                    interpolator_tau_c(norm_b) * np.cos(4 * polar_b),
                    phi_angle,
                    axis=2,
                )
                int_tau_b_sin = integrate.simpson(
                    interpolator_tau_b(norm_a) * np.sin(4 * polar_a),
                    phi_angle,
                    axis=2,
                )
                int_tau_c_sin = integrate.simpson(
                    interpolator_tau_c(norm_b) * np.sin(4 * polar_b),
                    phi_angle,
                    axis=2,
                )
                int_xi_cos = integrate.simpson(
                    interpolator_xi(norm_a) * np.cos(4 * polar_a),
                    phi_angle,
                    axis=2,
                )
                int_rho_cos = integrate.simpson(
                    interpolator_rho(norm_b) * np.cos(4 * polar_b),
                    phi_angle,
                    axis=2,
                )
                int_xi_sin = integrate.simpson(
                    interpolator_xi(norm_a) * np.sin(4 * polar_a),
                    phi_angle,
                    axis=2,
                )
                int_rho_sin = integrate.simpson(
                    interpolator_rho(norm_b) * np.sin(4 * polar_b),
                    phi_angle,
                    axis=2,
                )
                ...
                int_angle = (
                    int_tau_b_cos * int_tau_c_cos
                    + int_tau_b_sin * int_tau_c_sin
                    + int_xi_cos * int_rho_cos
                    + int_xi_sin * int_rho_sin
                )
