...
params = [params_des, params_sp, params_sp, params_axel]
...
for path_gal, path_psf, cat_id, param, mask, square_size in zip(
    ...
    try:
        ...
        if param is None:
            ...
            rho_stat_handler.catalogs.set_params(
                param, output
        ...
        )  # Compute and save the rho statistics

filenames = ["rho_stats_" + cat_id + ".fits" for cat_id in catalog_ids]

rho_stat_handler.plot_rho_stats(
    filenames, colors, catalog_ids, abs=True, savefig="rho_stats.png"
)  # Plot
...
for path_gal, path_psf, cat_id, param, mask, square_size in zip(
    ...
    try:
        ...
        if param is None:
            ...
            tau_stat_handler.catalogs.set_params(
                param, output
            )  # Set the parameters
        ...
        only_p = lambda corrs: np.array(
            [corr.xip for corr in corrs]
        ).flatten()  # function to extract the tau+
        tau_stat_handler.compute_tau_stats(
            cat_id,
            "tau_stats_" + cat_id + ".fits",
            save_cov=True,
            func=only_p,
            var_method="jackknife",
        )  # Compute and save the tau statistics


filenames = ["tau_stats_" + cat_id + ".fits" for cat_id in catalog_ids]
tau_stat_handler.plot_tau_stats(
    filenames, colors, catalog_ids, savefig="tau_stats.png", plot_tau_m=False
)  # Plot
...
for cat_id, param in zip(catalog_ids, params):
    ...
    flat_samples, mcmc_result, q = psf_fitter.run_chain(
        savefig="mcmc_samples_" + cat_id + ".png",
        npatch=param["patch_number"],
        apply_debias=True,
    )
...
plot_contours(
    ...
    savefig=output + "/contours_tau_stat.png",
    legend_labels=catalog_ids,
...
for mcmc_result, cat_id, color, flat_sample in zip(
    mcmc_result_list, catalog_ids, colors, flat_sample_list
):
    psf_fitter.load_rho_stat("rho_stats_" + cat_id + ".fits")
    for i in range(100):
        psf_fitter.plot_xi_psf_sys(
            flat_sample[-i + 1], cat_id, color, alpha=0.1
        )
