...
for path_gal, path_psf, cat_id, param, mask in zip(paths_gal, paths_psf, catalog_ids, params, masks): #Iterate on the different catalogs
    try:
        ...
        rho_stat_handler.build_cat_to_compute_rho(path_psf, catalog_id=cat_id, mask=mask) #Build the different catalogs
...
for path_gal, path_psf, cat_id, param, mask in zip(paths_gal, paths_psf, catalog_ids, params, masks): #Iterate on the catalogs
    ...
    try:
        ...
        if 'psf_'+cat_id not in tau_stat_handler.catalogs.catalogs_dict.keys():
            tau_stat_handler.build_cat_to_compute_tau(path_psf, cat_type='psf', catalog_id=cat_id, mask=mask) #Build the catalog of galaxies. PSF was computed above
        tau_stat_handler.build_cat_to_compute_tau(path_gal, cat_type='gal', catalog_id=cat_id, mask=mask) #Build the catalog of galaxies. PSF was computed above
